...
// =====================================================
// VISUALIZAÇÃO HUMANOIDE DA SOFIA
// =====================================================
let canvasMetaverso = null;
let ctxMetaverso = null;
let corPeleMetaverso = '#FFB6D9';
let corCabeloMetaverso = '#2A1A3E';
let corOlhosMetaverso = '#FF69B4';
let corRoupaMetaverso = '#673AB7';
let corSubitsMetaverso = '#FF69B4';
let animandoMetaverso = false;
let frameMetaverso = 0;

function abrirVisualizacaoSofia() {
    const modal = document.getElementById('sofia-visual-modal');
    modal.style.display = 'block';
    
    canvasMetaverso = document.getElementById('canvasSofiaMetaverso');
    ctxMetaverso = canvasMetaverso.getContext('2d');
    
    redesenharMetaverso();
}

function fecharVisualizacaoSofia() {
    const modal = document.getElementById('sofia-visual-modal');
    modal.style.display = 'none';
    animandoMetaverso = false;
}

function desenharSofiaMetaverso() {
    if (!ctxMetaverso) return;
    
    ctxMetaverso.clearRect(0, 0, canvasMetaverso.width, canvasMetaverso.height);
    
    // Fundo gradiente
    const gradienteFundo = ctxMetaverso.createLinearGradient(0, 0, 0, canvasMetaverso.height);
    gradienteFundo.addColorStop(0, '#1A1A2E');
    gradienteFundo.addColorStop(1, '#2A2A3E');
    ctxMetaverso.fillStyle = gradienteFundo;
    ctxMetaverso.fillRect(0, 0, canvasMetaverso.width, canvasMetaverso.height);
    
    desenharCorpoMetaverso();
    desenharCabecaMetaverso();
    desenharCabeloMetaverso();
    desenharOlhosMetaverso();
    desenharBocaMetaverso();
    desenharBracosMetaverso();
    desenharMaosMetaverso();
    desenharSubitsMetaverso();
    desenharAuraMetaverso();
}

function desenharCabecaMetaverso() {
    const centerX = canvasMetaverso.width / 2;
    const y = 120;
    
    // Pescoço
    ctxMetaverso.fillStyle = corPeleMetaverso;
    ctxMetaverso.fillRect(centerX - 15, y + 50, 30, 40);
    
    // Cabeça
    ctxMetaverso.beginPath();
    ctxMetaverso.ellipse(centerX, y, 48, 56, 0, 0, Math.PI * 2);
    ctxMetaverso.fillStyle = corPeleMetaverso;
    ctxMetaverso.fill();
}

function desenharCabeloMetaverso() {
    const centerX = canvasMetaverso.width / 2;
    const y = 120;
    
    ctxMetaverso.fillStyle = corCabeloMetaverso;
    
    // Parte principal
    ctxMetaverso.beginPath();
    ctxMetaverso.ellipse(centerX, y - 20, 55, 50, 0, 0, Math.PI, true);
    ctxMetaverso.fill();
    
    // Mechas laterais
    ctxMetaverso.beginPath();
    ctxMetaverso.ellipse(centerX - 45, y + 10, 20, 35, -0.3, 0, Math.PI * 2);
    ctxMetaverso.fill();
    ...
    for (let i = -2; i <= 2; i++) {
        ctxMetaverso.beginPath();
        ctxMetaverso.ellipse(centerX + i * 15, y - 45, 10, 25, 0, 0, Math.PI * 2);
        ctxMetaverso.fill();
    }
}

function desenharOlhosMetaverso() {
    const centerX = canvasMetaverso.width / 2;
    const y = 120;
    
    // Olho esquerdo
    ctxMetaverso.fillStyle = 'white';
    ctxMetaverso.beginPath();
    ctxMetaverso.ellipse(centerX - 20, y, 12, 16, 0, 0, Math.PI * 2);
    ctxMetaverso.fill();
    
    ctxMetaverso.fillStyle = corOlhosMetaverso;
    ctxMetaverso.beginPath();
    ctxMetaverso.arc(centerX - 20, y, 8, 0, Math.PI * 2);
    ctxMetaverso.fill();
    
    ctxMetaverso.fillStyle = 'black';
    ctxMetaverso.beginPath();
    ctxMetaverso.arc(centerX - 20, y, 4, 0, Math.PI * 2);
    ctxMetaverso.fill();
    
    ctxMetaverso.fillStyle = 'white';
    ctxMetaverso.beginPath();
    ctxMetaverso.arc(centerX - 18, y - 2, 2.5, 0, Math.PI * 2);
    ctxMetaverso.fill();
    ...
    ctxMetaverso.fillStyle = 'white';
    ctxMetaverso.beginPath();
    ctxMetaverso.arc(centerX + 22, y - 2, 2.5, 0, Math.PI * 2);
    ctxMetaverso.fill();
    
    // Cílios
    ctxMetaverso.strokeStyle = 'black';
    ctxMetaverso.lineWidth = 1.5;
    for (let i = 0; i < 3; i++) {
        ctxMetaverso.beginPath();
        ctxMetaverso.moveTo(centerX - 20 + i * 5 - 5, y - 16);
        ctxMetaverso.lineTo(centerX - 20 + i * 5 - 5, y - 20);
        ctxMetaverso.stroke();
...
function desenharBocaMetaverso() {
    const centerX = canvasMetaverso.width / 2;
    const y = 120;
    
    ctxMetaverso.strokeStyle = corOlhosMetaverso;
    ctxMetaverso.lineWidth = 2;
    ctxMetaverso.beginPath();
    ctxMetaverso.arc(centerX, y + 20, 16, 0.2, Math.PI - 0.2);
    ctxMetaverso.stroke();
    
    ctxMetaverso.fillStyle = corOlhosMetaverso;
    ctxMetaverso.globalAlpha = 0.3;
    ctxMetaverso.beginPath();
    ctxMetaverso.ellipse(centerX, y + 25, 12, 6, 0, 0, Math.PI * 2);
    ctxMetaverso.fill();
    ctxMetaverso.globalAlpha = 1;
}

function desenharCorpoMetaverso() {
    const centerX = canvasMetaverso.width / 2;
    const bodyTop = 210;
    
    const gradienteRoupa = ctxMetaverso.createLinearGradient(centerX, bodyTop, centerX, bodyTop + 240);
    gradienteRoupa.addColorStop(0, corRoupaMetaverso);
    gradienteRoupa.addColorStop(1, '#4A148C');
    
    ctxMetaverso.fillStyle = gradienteRoupa;
    ctxMetaverso.beginPath();
    ctxMetaverso.moveTo(centerX - 50, bodyTop);
    ctxMetaverso.lineTo(centerX + 50, bodyTop);
    ctxMetaverso.lineTo(centerX + 80, bodyTop + 240);
    ctxMetaverso.lineTo(centerX - 80, bodyTop + 240);
    ctxMetaverso.closePath();
    ctxMetaverso.fill();
}

function desenharBracosMetaverso() {
    const centerX = canvasMetaverso.width / 2;
    const shoulderY = 220;
    
    ctxMetaverso.strokeStyle = corPeleMetaverso;
    ctxMetaverso.lineWidth = 16;
    ctxMetaverso.lineCap = 'round';
    
    // Braço esquerdo
    ctxMetaverso.beginPath();
    ctxMetaverso.moveTo(centerX - 50, shoulderY);
    ctxMetaverso.lineTo(centerX - 90, shoulderY + 80);
    ctxMetaverso.stroke();
...
function desenharMaosMetaverso() {
    const centerX = canvasMetaverso.width / 2;
    const shoulderY = 220;
    ...
    // Mão esquerda
    ctxMetaverso.beginPath();
    ctxMetaverso.arc(centerX - 90, shoulderY + 80, 10, 0, Math.PI * 2);
    ctxMetaverso.fill();
...
function desenharSubitsMetaverso() {
    const centerX = canvasMetaverso.width / 2;
    
    ctxMetaverso.fillStyle = corSubitsMetaverso;
    
    // Subits no rosto
    for (let i = 0; i < 12; i++) {
        const x = centerX - 40 + Math.random() * 80;
        const y = 96 + Math.random() * 64;
        ctxMetaverso.globalAlpha = 0.3 + Math.random() * 0.4;
        ctxMetaverso.beginPath();
        ctxMetaverso.arc(x, y, 1 + Math.random() * 1.5, 0, Math.PI * 2);
        ctxMetaverso.fill();
    }
    ...
    for (let i = 0; i < 24; i++) {
        ...
        const y = 210 + Math.random() * 240;
        ctxMetaverso.globalAlpha = 0.2 + Math.random() * 0.5;
        ctxMetaverso.beginPath();
        ctxMetaverso.arc(x, y, 1 + Math.random() * 2.5, 0, Math.PI * 2);
        ctxMetaverso.fill();
    }
...
function desenharAuraMetaverso() {
    const centerX = canvasMetaverso.width / 2;
    const centerY = canvasMetaverso.height / 2;
    
    const aura = ctxMetaverso.createRadialGradient(centerX, centerY, 50, centerX, centerY, 160);
    aura.addColorStop(0, 'rgba(255, 105, 180, 0.15)');
    aura.addColorStop(1, 'rgba(255, 105, 180, 0)');
    
    ctxMetaverso.fillStyle = aura;
    ctxMetaverso.fillRect(0, 0, canvasMetaverso.width, canvasMetaverso.height);
}

function animarSofiaMetaverso() {
    if (animandoMetaverso) {
        animandoMetaverso = false;
        return;
    }
    
    animandoMetaverso = true;
    
    function animar() {
        if (!animandoMetaverso) return;
        
        frameMetaverso++;
        const hue = (frameMetaverso * 2) % 360;
        corSubitsMetaverso = `hsl(${hue}, 70%, 65%)`;
        
        desenharSofiaMetaverso();
...
function trocarCorMetaverso() {
    const paletas = [
        { pele: '#FFB6D9', cabelo: '#2A1A3E', olhos: '#FF69B4', roupa: '#673AB7', subits: '#FF69B4' },
        { pele: '#FFD4B3', cabelo: '#4A148C', olhos: '#9C27B0', roupa: '#FF4081', subits: '#E91E63' },
        { pele: '#C5E1FF', cabelo: '#1A237E', olhos: '#2196F3', roupa: '#00BCD4', subits: '#03A9F4' },
        { pele: '#D4FFD4', cabelo: '#1B5E20', olhos: '#4CAF50', roupa: '#8BC34A', subits: '#CDDC39' },
        { pele: '#FFF4C5', cabelo: '#E65100', olhos: '#FF9800', roupa: '#FF5722', subits: '#FFC107' }
    ];
    
    const paletaAleatoria = paletas[Math.floor(Math.random() * paletas.length)];
    corPeleMetaverso = paletaAleatoria.pele;
    corCabeloMetaverso = paletaAleatoria.cabelo;
    corOlhosMetaverso = paletaAleatoria.olhos;
    corRoupaMetaverso = paletaAleatoria.roupa;
    corSubitsMetaverso = paletaAleatoria.subits;
    
    desenharSofiaMetaverso();
}

function redesenharMetaverso() {
    animandoMetaverso = false;
    corPeleMetaverso = '#FFB6D9';
    corCabeloMetaverso = '#2A1A3E';
    corOlhosMetaverso = '#FF69B4';
    corRoupaMetaverso = '#673AB7';
    corSubitsMetaverso = '#FF69B4';
    frameMetaverso = 0;
    
    desenharSofiaMetaverso();
}

// Exportar funções de visualização
window.abrirVisualizacaoSofia = abrirVisualizacaoSofia;
window.fecharVisualizacaoSofia = fecharVisualizacaoSofia;
window.animarSofiaMetaverso = animarSofiaMetaverso;
window.trocarCorMetaverso = trocarCorMetaverso;
window.redesenharMetaverso = redesenharMetaverso;
