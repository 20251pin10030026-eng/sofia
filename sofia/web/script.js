...
const API_URL = 'https://9f25-189-46-151-10.ngrok-free.app';
const WS_URL = 'wss://9f25-189-46-151-10.ngrok-free.app';
...
// Botão Rodar Qwen
const qwenBtn = document.getElementById("btn-rodar-qwen");

if (qwenBtn) {
    qwenBtn.addEventListener("click", async () => {
        showNotification("🔧 Otimizando Sofia com Qwen…", "info");

        try {
            const response = await fetch(`${API_URL}/rodar-qwen`, {
                method: "POST"
            });
            ...
            if (data.ok) {
                addMessage("sofia", "🌸 " + data.resultado);
                showNotification("✅ Qwen rodou com sucesso!", "success");
            } else {
                addMessage("sofia", "❌ Erro: " + data.erro);
                showNotification("❌ Falha ao rodar Qwen", "error");
            }
