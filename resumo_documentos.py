...
# Listar documentos de Sofia e teorias científicas
docs_sofia = memoria.listar_aprendizados("documentos_sofia")
teorias = memoria.listar_aprendizados("teorias_cientificas")

todos_docs = {}
if docs_sofia:
    todos_docs["documentos_sofia"] = docs_sofia
if teorias:
    todos_docs["teorias_cientificas"] = teorias
...
if not todos_docs:
    ...
    for categoria, docs in todos_docs.items():
        ...
        for chave, dados in docs.items():
            ...
            print(f"\n📖 Preview (primeiros 250 caracteres):")
            if isinstance(valor, dict) and 'conteudo' in valor:
                preview = valor['conteudo'][:250]
            else:
                preview = str(valor)[:250]
            print(f"   {preview}...")
            print()
